...
extern	uint8_t recieve_temp_report;
	...
	if (!quiet)
     ...
     if (queued_data) 
        ...
        printf("Command %c will be sent on next temperature report",message_type);      
        }
	...
	monitorControlTime = time(NULL);
				...
				printf("\nsending identify ");
				uint8_t* buf;
				buf = HRF_make_FSK_msg(manufacturerId, encryptId, productId, sensorId,
											  2, 0xBF, 0);
				//HRF_send_FSK_msg(buf, encryptId);
